...
import { Badge, Card, EmptyState, ProgressBar, SectionHeader, StatCard, Toast, TrustStrip } from "../components/ui";
  ...
  return (
    <div className="page-stack">
      <div className="page-title-row">
        ...
        <button className="primary-button" type="button" onClick={() => navigate("/donate")}>
      ...
      <div className="dashboard-grid">
        <Card className="campaign-card">
          ...
          <button className="primary-button full" type="button" onClick={() => navigate("/donate")}>Donate to Campaign</button>
        </Card>
...
function DonateView() {
  ...
  const [fund, setFund] = useState("Sadaqah");
  const [frequency, setFrequency] = useState("Monthly");

  function submitDonation() {
    downloadSimplePdf("masjidpro-donor-receipt.pdf", "MasjidPro Donation Receipt", [
      `Amount: ${currency(amount)}`,
      `Fund: ${fund}`,
      `Frequency: ${frequency}`,
      "Payment method: Card",
      "Status: Confirmed",
      "Receipt generated by MasjidPro",
    ]);
    setToast("Donation confirmed and receipt downloaded.");
  }
    ...
    <div className="page-stack">
      ...
      <Card>
        ...
        <div className="form-grid two">
          <label><span>Amount</span><input value={amount} type="number" onChange={(event) => setAmount(Number(event.target.value))} /></label>
          <label><span>Fund</span><select value={fund} onChange={(event) => setFund(event.target.value)}><option>Zakat</option><option>Sadaqah</option><option>General</option><option>Building</option></select></label>
          <label><span>Frequency</span><select value={frequency} onChange={(event) => setFrequency(event.target.value)}><option>One-time</option><option>Monthly</option><option>Weekly</option></select></label>
          <label><span>Payment method</span><select defaultValue="Card"><option>Card</option><option>Bank Transfer</option></select></label>
        </div>
        <button className="primary-button full" type="button" onClick={submitDonation}>Donate and Generate Receipt</button>
      </Card>
...
function ReceiptsView() {
  function downloadReceipt(donation: (typeof donations)[number]) {
    downloadSimplePdf(`${donation.receiptId}.pdf`, "MasjidPro Donation Receipt", [
        ...
        <div className="receipt-list">
          {donations.filter((donation) => donation.receiptId !== "Pending").map((donation) => (
            <div className="receipt-row" key={donation.id}>
              ...
              <button className="secondary-button compact" type="button" onClick={() => downloadReceipt(donation)}><Download size={16} /> PDF</button>
            </div>
          ))}
        </div>
      </Card>
  ...
  return (
    <div className="page-stack">
      ...
      <Card>
        ...
        <button className="primary-button full" type="button" onClick={() => setToast("Profile preferences saved.")}>Save Profile</button>
